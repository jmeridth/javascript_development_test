...
Please install any dependencies with node and create the `node_modules` folder. The .gitignore by default will ignore this directory, but if you have an additional IDE files or folders created please add them to your `.gitignore` or specifically do not add them into the git repo. When complete, you **must generate** a `package.lock` using node. If you are not familiar with node, you can find several guides online.
...
1. Source Code
  * The source code should be in JavaScript, using version 18+ (or any major/minor revision thereafter)
  * Unit tests (use framework of your choice)
...
```
$ javascript/npm grocery.js input.txt
...
Here is the expected command output:
```
$ javascrip/npm  grocery.js input.txt
...
Here is the expected command output:
```
$ javascript/npm grocery.py input.txt
